...
const AssigneesDropdown = ({ assignees, availableMembers, onAssigneesChange }) => {
            ...
            {availableMembers.map(member => (
              ...
              >
                <div className="flex items-center flex-1">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white font-medium !mr-2 ${member.bgColor}`}>
                    {member.initials}
                  </div>
        ...
        <div className="space-y-3">
          {assignees.map(member => (
            <div key={member.id} className="flex items-center group">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-medium !mr-3 ${member.bgColor}`}>
                {member.initials}
              </div>
              <div className="font-medium text-gray-900 flex-1">{member.name}</div>
              <button
                onClick={() => {
                  const newAssignees = assignees.filter(m => m.id !== member.id);
                  onAssigneesChange(newAssignees);
                }}
