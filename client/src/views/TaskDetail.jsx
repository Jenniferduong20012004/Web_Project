...
import SubtaskList from "../component/board/SubtaskList";
import AssetsList from "../component/board/AssetsList";
import AssigneesDropdown from "../component/board/AssigneesDropdown";
          ...
          <div className="bg-white rounded-lg shadow !p-6 !mb-6">
              ...
              {editMode.title ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="text-2xl font-bold text-gray-900 border-b border-blue-500 focus:outline-none w-full !pb-1"
                    autoFocus
                  />
                  <button
              ...
              <div className="flex items-center !mt-2">
                ...
                <div className="relative" ref={statusDropdownRef}>
                  <div 
                    ...
                    onClick={() => setEditMode({...editMode, status: !editMode.status})}
                  >
                    <span className={`h-2 w-2 rounded-full ${getStatusColor(task.status)} !mr-2`}></span>
                    <span className="text-sm text-gray-500 uppercase">{task.status.replace("-", " ")}</span>
                ...
                <div className="relative ml-4" ref={priorityDropdownRef}>
                  <div 
                    className={`text-xs !px-2 !py-1 rounded-full cursor-pointer flex items-center ${getPriorityBg(task.priority)}`}
                    onClick={() => setEditMode({...editMode, priority: !editMode.priority})}
                  >
                    {task.priority}
                  ...
                  {editMode.priority && (
                    <div className="absolute top-full left-0 mt-1 bg-white shadow-lg rounded-md border border-gray-200 z-10 w-32">
                      {priorityOptions.map(option => (
                        <div 
                          key={option.id}
                          className={`!px-3 !py-2 text-sm hover:bg-gray-50 cursor-pointer ${option.class.includes('text-') ? option.class.split(' ').find(c => c.startsWith('text-')) : ''}`}
                          onClick={() => handleSaveField('priority', option.id)}
                        >
                          {option.label}
                        </div>
                      ))}
              ...
              {editMode.dueDate ? (
                <div className="flex items-center gap-2 !mt-2">
                  <input
                    ...
                    onBlur={() => handleSaveField('dueDate', task.dueDate)}
                    onKeyDown={(e) => handleKeyDown(e, 'dueDate', task.dueDate)}
                    className="text-sm text-gray-600 border-b border-gray-300 focus:border-blue-500 focus:outline-none !py-1"
                  />
                  <button
                    onClick={() => handleSaveField('dueDate', task.dueDate)}
                    className="text-blue-600 hover:text-blue-800"
                  >
                ...
                <div 
                  className="text-sm text-gray-500 !mt-2 cursor-pointer hover:bg-gray-50 !py-1 !px-2 rounded-md inline-block"
                  onClick={() => setEditMode({...editMode, dueDate: true})}
                >
                  Due to: {task.dueDate}
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {editMode.description ? (
                  <div className="flex flex-col gap-2">
                    <textarea
                      ...
                      onBlur={() => handleSaveField('description', task.description)}
                      onKeyDown={(e) => {
                    ...
                    <div className="flex justify-end">
                      <button
                        onClick={() => handleSaveField('description', task.description)}
              ...
              <div>
                {/* Assigned Members */}
                <div className="!mb-6">
                  <h3 className="font-medium text-gray-900 !mb-3">Assigned members:</h3>
                  <AssigneesDropdown 
