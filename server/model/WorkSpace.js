...
class WorkSpace {
  ...
  static createForManager(userId, workSpaceData, callback) {
    ...
    WorkSpace.create(workSpaceData, (err, result) => {
      ...
      const query = "INSERT INTO joinWorkSpace (isPending, isManager, dateJoin, userId, WorkSpace) VALUES (?, ?, ?, ?, ?)";
      pool.query(query, [false, true, dateCreate, userId, result.id], (e, r) => {
        if (e) {
          console.error("Error linking workspace to manager:", e);
          return callback(e, null);
        }
        console.log("Workspace created and linked to manager successfully!");
        return callback(null, { joinId: r.insertId, userId, workspaceId: result.id });
      });
    });
  }
}
